    ...
    <form action="{{ $data['action'] ?? '/login' }}" method="POST" class="space-y-4">
        ...
        @if(!empty($data['forgot_url']) || !empty($data['register_url']))
            <div class="flex justify-between text-sm text-gray-500 dark:text-gray-400">
                @if(!empty($data['forgot_url']))<a href="{{ $data['forgot_url'] }}" class="hover:text-blue-600 dark:hover:text-blue-400">Forgot password?</a>@endif
                @if(!empty($data['register_url']))<a href="{{ $data['register_url'] }}" class="hover:text-blue-600 dark:hover:text-blue-400">Create account</a>@endif
            </div>
        @endif
    </form>
</div>
