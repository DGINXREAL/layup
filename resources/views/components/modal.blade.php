...
>
    ...
    <template x-teleport="body">
        <div x-show="open" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="open = false" @keydown.escape.window="open && (open = false)">
            ...
            <div class="relative bg-white rounded-xl shadow-2xl w-full p-6 overflow-y-auto max-h-[90vh]" style="max-width: {{ $maxW }}" x-transition>
                <button @click="open = false" class="absolute top-3 right-3 text-gray-400 hover:text-gray-600 text-xl">&times;</button>
                @if(!empty($data['title']))
