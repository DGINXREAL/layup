...
>
    ...
    <div class="flex items-center justify-center gap-3 mb-8">
        <span class="text-sm font-medium" :class="!annual ? 'opacity-100' : 'opacity-50'">{{ $data['monthly_label'] ?? 'Monthly' }}</span>
    ...
    <div style="display:grid;grid-template-columns:repeat({{ min(count($plans), 4) }},1fr);gap:1.5rem">
        @foreach($plans as $plan)
            <div class="border rounded-xl p-6 text-center {{ !empty($plan['featured']) ? 'ring-2 relative' : '' }}" @if(!empty($plan['featured'])) style="ring-color: {{ $color }}" @endif>
                @if(!empty($plan['featured']))
                    <span class="absolute -top-3 left-1/2 -translate-x-1/2 text-xs text-white px-3 py-1 rounded-full" style="background-color: {{ $color }}">Popular</span>
                @endif
                ...
                <div class="text-3xl font-bold mb-4">
                    ...
                    <span class="text-sm font-normal text-gray-500" x-text="annual ? '/year' : '/month'"></span>
                </div>
                ...
                @if(!empty($plan['cta_url']))
                    <a href="{{ $plan['cta_url'] }}" class="block text-white font-medium py-2 rounded-lg hover:opacity-90 transition-opacity" style="background-color: {{ $color }}">{{ $plan['cta_text'] ?? 'Get Started' }}</a>
                @endif
            </div>
        @endforeach
    </div>
