...
it('make() creates instance', function () {
    $row = Row::make();
    expect($row)->toBeInstanceOf(Row::class);
});

it('getChildren/setChildren/addChild/hasChildren work', function () {
    $row = Row::make();
...
it('getData() returns data', function () {
    $row = Row::make(['gap' => 'gap-4']);
    expect($row->getData())->toBe(['gap' => 'gap-4']);
});

it('getFormSchema() returns array with Tabs', function () {
...
it('getContentFormSchema() returns empty by default for Column', function () {
...
it('getDesignFormSchema() returns spacing + background fields', function () {
